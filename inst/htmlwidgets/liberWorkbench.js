(function () {
  ...
  function highlightCode(source) {
    ...
    var pattern = /(#.*$|\/\/.*$|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|\b\d+(?:\.\d+)?(?:[eE][+-]?\d+)?\b|\b(?:if|else|for|while|return|TRUE|FALSE|NA|NULL)\b|\b(?:THETA|ETA|ERR|EPS|A|DADT|S|F|Y|IPRED|PRED|TIME|T|AMT|CMT|EVID)\b|\$[A-Z][A-Z0-9_]*)/gm;
    text.replace(pattern, function (token, match, offset) {
      output += escapeCode(text.slice(cursor, offset));
      var kind = /^#|^\/\//.test(token) ? "comment" : /^['\"]/.test(token) ? "string" : /^\d/.test(token) ? "number" : /^\$/.test(token) ? "block" : /^(if|else|for|while|return|TRUE|FALSE|NA|NULL)$/.test(token) ? "keyword" : "symbol";
