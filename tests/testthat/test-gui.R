...
test_that("legacy workbench layout and workflow controls are present", {
  ...
  expect_match(source, 'The covariance step reuses the fitted integration design', fixed = TRUE)
  expect_match(source, 'common-design importance subject scores', fixed = TRUE)
  expect_match(source, 'Post-fit observed marginal information', fixed = TRUE)
...
test_that("workbench content remains scrollable in short viewports", {
  ...
  expect_match(
    dependency,
    paste0("version: ", as.character(utils::packageVersion("LibeRation"))),
    fixed = TRUE
  )
})
